...
pub(crate) struct VariantNormalizer {
    fasta: FastaIndex,
}

impl VariantNormalizer {
    pub(crate) fn open(reference: &Path) -> Result<Self> {
        Ok(Self {
            fasta: open_fasta_index(reference)?,
        })
    }

    pub(crate) fn normalize_key(&mut self, key: VariantKey) -> Result<VariantKey> {
...
pub(crate) fn normalized_variant_keys_from_fields(
    fields: &[&str],
    mut normalizer: Option<&mut VariantNormalizer>,
) -> Result<Vec<VariantKey>> {
    let mut keys = Vec::new();
    for key in variant_keys_from_fields(fields) {
